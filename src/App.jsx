...
function App() {
  ...
  // Function to get next available number for each type
  const getNextId = (type) => {
    // Collect all existing IDs of the given type
    const ids = [];
    ...
    (event) => {
      ...
      if (active.id.startsWith("sidebar-")) {
        ...
        if (itemType === "row") {
          ...
          // Add new column - find parent row
          const newColumn = {
            type: "COLUMN",
            id: getNextId("column"),
            height: 200,
          ...
          // Find target row (either dropped on row or column)
          let targetRowId = null;

          if (over.id.startsWith("row")) {
            targetRowId = over.id;
          } else if (over.id.startsWith("column")) {
            // Find which row contains this column
      ...
      // Handle reordering existing components
        ...
        // Find source column containing the component
  ...
  // Update JSON data whenever layout changes
  useEffect(() => {
